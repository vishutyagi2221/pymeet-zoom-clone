import { MessageSquare, Mic, MicOff, MonitorUp, PhoneOff, Users, Video, VideoOff } from "lucide-react";
import { Button } from "./Button";

export function MeetingControls({ isHost, micEnabled, cameraEnabled, screenSharing, onToggleMic, onToggleCamera, onShareScreen, onToggleChat, onToggleParticipants, onLeave }: { isHost: boolean; micEnabled: boolean; cameraEnabled: boolean; screenSharing: boolean; onToggleMic: () => void; onToggleCamera: () => void; onShareScreen: () => void; onToggleChat: () => void; onToggleParticipants: () => void; onLeave: () => void }) {
  ...
  return (
    <div className="fixed bottom-5 left-1/2 z-30 flex -translate-x-1/2 items-center gap-2 rounded-lg border border-line bg-slate-950/80 p-2 shadow-soft backdrop-blur-xl">
      ...
      {isHost && <Button title="Share screen" variant={screenSharing ? "primary" : "secondary"} className={item} onClick={onShareScreen}><MonitorUp size={19} /></Button>}
