...
export function Dashboard() {
  ...
  const join = async () => {
    if (!meetingId.trim() || joining) return;
    setJoinError("");
    setJoining(true);
    try {
    ...
    <div className="bg-premium min-h-screen text-white"><Navbar />
      <main className="mx-auto max-w-7xl px-4 py-8">
        ...
        <section className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
          ...
          <Card className="p-6"><h2 className="text-2xl font-bold">Join Meeting</h2><p className="mt-2 text-slate-400">Enter a meeting ID shared by your host.</p><div className="mt-6 flex flex-col gap-2 sm:flex-row"><input value={meetingId} onChange={(e) => setMeetingId(e.target.value.toUpperCase())} className="min-w-0 flex-1 rounded-lg border border-line bg-white/10 px-4 py-3 outline-none focus:border-cyan-300" placeholder="ABCD-EF12-3456" /><Button disabled={joining} onClick={join} className="sm:min-w-24">{joining ? "Joining..." : "Join"}</Button></div>{joinError && <p className="mt-2 text-sm text-red-400">{joinError}</p>}</Card>
        </section>
